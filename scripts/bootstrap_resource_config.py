...
def infer_shell() -> ShellType:
    """Infers the user's default shell configuration file."""
    shell = pathlib.Path(pwd.getpwuid(os.getuid()).pw_shell)
    if not shell.exists():
        raise RuntimeError("Corrupt login shell path")
    shell = shell.resolve() # Login shells can be symlinks
    shell = shell.name
    shell_config_map = {
        "zsh":   ShellType.ZSH,
        "bash": ShellType.BASH,
    ...
    for key, shelltype in shell_config_map.items():
        if key == shell:
            print(f"Detected shell: {key}. Using config file: {shelltype.config_file()}")
            return shell_type[key]

    print(
        "Could not infer the default shell. Please specify the shell and configuration file manually."
    )
    print("0: Zsh (~/.zshrc)")
    print("1: Bash (~/.bashrc)")
    print("2: FiSH (~/.config/fish/config.fish)")
    print("3: Tcsh (~/.tcshrc)")
    answer = None
    while answer == None:
        answer = input("What is your shell? (If it doesn't appear here, it's unfortunately not supported yet.)\n>> ")
        try:
            answer = int(answer)
    ...
    match answer:
        case 0:
            return (ShellType.ZSH, "~/.zshrc")
        case 1:
            return (ShellType.BASH, "~/.bashrc")
        case 2:
            return (ShellType.FISH, "~/.config/fish/config.fish")
        case 3:
            return (ShellType.TCSH, "~/.tcshrc")
        case _:
            raise RuntimeError("Internal error: unexpected value for shell type")
...
def update_shell_config(
    ...
    shell_config = os.path.expanduser(shell_type.config_file())
    start_marker = "# ====== GiGL ENV Config - Begin ====="
    end_marker = "# ====== GiGL ENV Config - End ====="
    export_lines = ""
    match shell_type:
        case ShellType.BASH | ShellType.ZSH | ShellType.FISH:
            export_lines = [
                start_marker + "\n",
        ...
        case ShellType.TCSH:
            export_lines = [
                start_marker + "\n",
                "# This section is auto-generated by GiGL/scripts/bootstrap_resource_config.py.\n",
                f'setenv GIGL_TEST_DEFAULT_RESOURCE_CONFIG "{gigl_test_default_resource_config}"\n',
                f'setenv GIGL_PROJECT "{gigl_project}"\n',
                f'setenv GIGL_DOCKER_ARTIFACT_REGISTRY "{gigl_docker_artifact_registry_path}"\n',
                end_marker + "\n",
            ]
    ...
    if not os.path.exists(shell_config):
        raise FileNotFoundError(
            f"Shell config file '{shell_config}' does not exist."
        )
    shell_config_lines: list[str]
    with open(shell_config, "r") as shell_config:
    ...
    with open(shell_config, "w") as shell_config:
        shell_config.writelines(updated_shell_config_lines)

    print(f"Updated {shell_config} with:\n{export_lines}.")
...
if __name__ == "__main__":
    ...
    if should_update_shell_config == "y":
        ...
        print(
            f"Please restart your shell or run `source {shell_type}` to apply the changes."
        )
    else:
        print(
